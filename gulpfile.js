...
gulp.task('browserSync', function() {
  ...
  gulp.watch("master/css/*.css").on('change', browserSync.reload);
  gulp.watch("master/images/*.+(jpg|jpeg|png|gif|svg)").on('change', browserSync.reload);
  gulp.watch("master/script/*.js").on('change', browserSync.reload);
});
...
gulp.task('sass', function() { // Создаем таск "sass"
  return gulp.src(['src/scss/**/*.scss']) // Берем источник
    ...
    .pipe(gulp.dest('master/css')) // Выгружаем результата в папку css
    .pipe(browserSync.stream());
  });

gulp.task("scripts", function() {
    return gulp.src("src/script/*.js") // директория откуда брать исходники
        .pipe(concat('scripts.js')) // объеденим все js-файлы в один 
        .pipe(uglify()) // вызов плагина uglify - сжатие кода
        .pipe(rename({ suffix: '.min' })) // вызов плагина rename - переименование файла с приставкой .min
        .pipe(gulp.dest("master/script")); // директория продакшена, т.е. куда сложить готовый файл
});
